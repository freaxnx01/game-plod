...
- Tutorial step guidance: a prompt banner for the current step, skill tiles
  locked to the one skill that step teaches, and a step that only advances once
  the step's skill has actually been used *and* a plod has cleared the matching
  obstacle (#4).
